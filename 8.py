...
axes.set_ylabel("Вольтаж, В", fontsize = 16)
axes.set_title("График зарядки-разрядки конденсатора", fontsize = 20)
...
charge_plot_line.set_label("Зарядка конденсатора")
discharge_plot_line.set_label("Разрядка конденсатора")
...
axes.grid(color = "white", which = "both", linestyle = ':', linewidth = 0.5)
...
axes.text(x=charge_time+0.1, y = 0.05, s=str(round(charge_time/1000, 2)), fontsize = 12)

axes.scatter(x = 0.0, y = volt_max, color = 'green')
axes.text(x = 0.1, y = volt_max+0.05, s = str(round(volt_max, 2)), fontsize = 12)

axes.text(x = (charge_time/2-0.8), y = volt_max/2, s = ("Время зарядки: " + str(round(charge_time/1000, 2)) + " с"), color = 'blue', fontsize = 14)
axes.text(x = (charge_time+discharge_time/2-0.8), y = volt_max/2, s = ("Время разрядки: " + str(round(discharge_time/1000, 2)) + " с"), color = 'red', fontsize = 14)
